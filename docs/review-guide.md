...
4. Open `/checkout` to compare every mock checkout outcome: allowed, blocked, pending admin review, pending document upload, ready for payment, payment failed, and paid.
5. Open `/checkout/address`, `/checkout/verification?case=blocked`, `/checkout/verification?case=pending_admin_review`, `/checkout/verification?case=pending_document_upload`, `/checkout/document-upload`, and `/checkout/payment` to inspect the full stepper.
6. Open `/checkout/success` and `/checkout/failed` to validate payment outcome states.
7. Open `/account`, `/account/orders`, `/account/orders/CG-1004`, and `/account/addresses` to inspect customer status tracking.
...
4. Review `/admin/orders`, `/admin/orders/CG-1003`, `/admin/verification-queue`, and `/admin/document-review` for manual review workflow and required notes.
...
## Mock/stubbed items
- All product, cart, order, verification, document, rule, launch-gate, and audit-log data is static mock data.
...
- No backend, database, file storage, identity provider, payment processor, or rule engine is connected.
