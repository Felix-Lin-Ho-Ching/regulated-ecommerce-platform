...
import { getEnabledShippingCarriers } from "@/lib/shipping/carriers";
...
export default async function PickPackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const carriers = canConfirmShipment ? await getEnabledShippingCarriers() : [];
  ...
  return (
    <AdminShell title={`Pick/pack ${order.orderNumber}`} currentPath="/admin/fulfillment">
      ...
      <section className="card mt-4 p-5"><h2 className="font-black">Shipment confirmation</h2>{order.fulfillmentStatus === "SHIPPED" ? <p className="mt-2 text-sm text-emerald-800">Shipped via {order.carrier || "—"} / {order.trackingNumber || "—"}.</p> : canConfirmShipment ? <SingleShipmentForm orderId={order.id} carriers={carriers} /> : <p className="mt-2 text-sm text-slate-600">Claim this paid order before confirming shipment. Fulfillment staff can only ship orders assigned to them.</p>}</section>
