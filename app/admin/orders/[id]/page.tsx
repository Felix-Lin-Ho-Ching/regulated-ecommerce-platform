    ...
    <div className="grid gap-4 xl:grid-cols-[2fr_1fr]">
      <section className="card p-5">
        ...
        <div className="grid gap-3 md:grid-cols-2 text-sm">
          <p><b>Status:</b> <StatusBadge tone={order.status === "READY_FOR_PAYMENT" ? "success" : "warning"}>{order.status}</StatusBadge></p><p><b>Order-request mode:</b> {order.paymentMode ?? "order_request"}</p>
          ...
          <p><b>Auto pipeline:</b> Order request received → auto eligibility passed → ready for payment</p><p><b>Fulfillment release:</b> Not released; do not fulfill until real payment exists.</p>
...
<div><OrderStatusForm orderId={order.id} orderNumber={order.orderNumber} currentStatus={order.status} statuses={adminOrderStatuses} />{order.status !== "SHIPPED" && order.status !== "CANCELLED" ? <CancelOrderForm orderId={order.id} /> : null}</div>
    ...
    <section className="card p-5 mt-4"><h2 className="font-black mb-3">Fulfillment token status</h2>{order.fulfillmentTokens.length ? <ul className="grid gap-2 text-sm">{order.fulfillmentTokens.map((token: any) => <li key={token.id}>Sent {fmt(token.createdAt)} · {token.usedAt ? `used ${fmt(token.usedAt)}` : token.expiresAt < new Date() ? "expired" : "active"}</li>)}</ul> : <p>No fulfillment token sent because payment has not been collected and fulfillment is not released.</p>}<p className="mt-3 text-sm text-slate-600">Resend fulfillment email/log by submitting a new order notification from the order pipeline if a token is expired or email logging failed.</p></section>
