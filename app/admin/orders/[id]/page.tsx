    ...
    <div className="grid gap-4 xl:grid-cols-[2fr_1fr]">
      <section className="card p-5">
        ...
        <div className="grid gap-3 text-sm md:grid-cols-2">
          ...
          <p><b>Fulfillment release:</b> Not released · {order.fulfillmentStatus}</p><p><b>Restricted item:</b> {restricted ? "Yes" : "No"}</p>
      ...
      <div><OrderStatusForm orderId={order.id} orderNumber={order.orderNumber} currentStatus={order.status} statuses={adminOrderStatuses.filter((s) => !["PAID", "FULFILLED", "SHIPPED"].includes(s))} />{canCancel ? <CancelOrderForm orderId={order.id} /> : null}<InternalNoteForm orderId={order.id} /><EmailActionsForm orderId={order.id} cancelled={order.status === "CANCELLED"} /></div>
    </div>
