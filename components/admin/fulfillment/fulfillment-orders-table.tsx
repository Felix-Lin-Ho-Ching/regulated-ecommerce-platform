  ...
  return (
    <div className="card overflow-x-auto">
      <table className="table">
        <thead><tr>{canOperateFulfillment ? <th>Action</th> : null}<th>Order</th><th>Customer</th><th>State/ZIP</th><th>Restricted</th><th>Item count</th><th>SKUs</th><th>Qty total</th><th>Fulfillment status</th><th>Assigned worker</th><th>Created</th><th>Shipped</th><th>Carrier/tracking</th></tr></thead>
        <tbody>{groups.map((group) => (<Fragment key={group.name}><tr><td colSpan={canOperateFulfillment ? 13 : 12} className="bg-slate-100 font-black">{group.name}</td></tr>{group.orders.map((order) => (<FulfillmentOrderRow key={order.id} admin={admin} order={order} />))}</Fragment>))}</tbody>
      </table>
