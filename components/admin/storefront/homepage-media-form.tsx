...
export function HomepageMediaForm({ media }: { media: HomepageHeroMedia }) {
  ...
  return <form action={action} className="card grid gap-4 p-5 md:grid-cols-2">
    ...
    <div className="grid gap-2"><Field label="Media URL" name="homepageUrl" value={media.url} error={state.errors.homepageUrl}/><p className="text-center text-xs font-black uppercase text-slate-500">or</p><label className="grid gap-2 text-sm font-bold text-slate-800">Or upload media file<input className="input" name="homepageUpload" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime"/><span className="text-xs font-medium text-slate-500">Upload is stored locally for development. Use external storage before production launch.</span></label></div>
    <div className="grid gap-2"><Field label="Fallback image URL" name="homepageThumbnailUrl" value={media.thumbnailUrl} error={state.errors.homepageThumbnailUrl}/><p className="text-center text-xs font-black uppercase text-slate-500">or</p><label className="grid gap-2 text-sm font-bold text-slate-800">Or upload fallback image<input className="input" name="homepageThumbnailUpload" type="file" accept="image/jpeg,image/png,image/webp"/><ErrorText message={state.errors.homepageThumbnailUpload}/></label></div>
