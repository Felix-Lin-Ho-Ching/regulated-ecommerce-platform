...
function PaymentBlock({ disabled }: { disabled: boolean }) {
  return <div className="mt-4 rounded-2xl border border-stone-200 bg-stone-50 p-4"><p className="font-bold text-slate-800">Payment is not collected online yet.</p><p className="mt-2 text-sm text-slate-600">Submit an order request now. We will review it before any fulfillment or payment step.</p><input type="hidden" name="paymentMode" value="order_request" /><fieldset disabled={disabled} className="mt-3"><label className="flex gap-3 text-sm font-bold"><input defaultChecked name="billingSame" type="checkbox" /> Use shipping address for order-request review</label></fieldset></div>;
}
