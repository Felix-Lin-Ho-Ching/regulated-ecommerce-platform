...
const fallbackProducts: CatalogProduct[] = mockProducts.map((product) => ({
  ...product,
  brand: "Stun Fry",
  reserved: Math.min(4, product.stock),
  features: [],
}));
...
export async function getCatalogProducts(): Promise<CatalogProduct[]> {
  ...
  return rows.map((product: CatalogProductRow) => {
    ...
    return {
      ...
      features: product.features.map((feature: CatalogProductRow["features"][number]) => ({
        code: feature.code,
        label: feature.label,
        value: feature.value,
