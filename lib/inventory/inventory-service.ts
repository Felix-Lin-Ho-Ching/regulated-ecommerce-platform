...
export type InventoryAvailability = { sku: string; onHand: number; reserved: number; available: number; canReserve: boolean };

export async function getInventoryAvailability(sku: string, quantity = 1): Promise<InventoryAvailability> {
  const variant = await prisma.productVariant.findUnique({ where: { sku }, include: { inventory: true } });
