...
export async function logOrderNotification(orderId: string, type: "customer_confirmation" | "admin_new_order" | "customer_cancellation"): Promise<{ error?: string }> {
  ...
  if (type === "customer_confirmation") {
    ...
    await logDebugEmail({ type: "CUSTOMER_CANCELLATION", to: customerEmail(order), subject: `Order request ${order.orderNumber} cancelled`, text: `Your order request ${order.orderNumber} has been cancelled. Payment was not collected and fulfillment was not released.`, orderId: order.id, metadata: { orderNumber: order.orderNumber, regeneratedByAdmin: true } });
  }
...
export async function cancelOrderBeforeShipment(orderId: string, note: string): Promise<{ error?: string }> {
  ...
  await logDebugEmail({ type: "CUSTOMER_CANCELLATION", to: customerEmail(order), subject: `Order request ${order.orderNumber} cancelled`, text: `Your order request ${order.orderNumber} has been cancelled. Payment was not collected and fulfillment was not released.`, orderId: order.id, metadata: { orderNumber: order.orderNumber } }).catch(() => undefined);
  await logDebugEmail({ type: "ADMIN_CANCELLATION", to: process.env.ADMIN_ORDER_EMAIL || process.env.ADMIN_EMAIL || "linhochingfelix@gmail.com", subject: `Order request ${order.orderNumber} cancelled`, text: `Order request ${order.orderNumber} was cancelled by admin. Active inventory reservations were released.`, orderId: order.id, metadata: { orderNumber: order.orderNumber } }).catch(() => undefined);
  revalidatePath("/admin/orders"); revalidatePath(`/admin/orders/${order.orderNumber}`); return {};
}
