...
export const checkoutCases: Record<CheckoutOutcome, { title:string; reason:string; state:string; orderId:string; payment:string; verification:string; fulfillment:string; docs:string[] }> = {
 ...
 blocked:{title:"Blocked by destination rule",reason:"Mock rule blocks knuckle_stun_device shipments to NY. Payment is unavailable.",state:"NY",orderId:"SF-1002",payment:"not_collected",verification:"blocked",fulfillment:"blocked",docs:[]},
 ...
 ready_for_payment:{title:"Ready for mock payment",reason:"Compliance and document review are approved. Continue to mock payment.",state:"TX",orderId:"SF-1005",payment:"ready",verification:"approved",fulfillment:"awaiting_payment",docs:[]},
 payment_failed:{title:"Mock payment failed",reason:"Processor returned a mock failure. Eligibility remains approved; retry payment.",state:"TX",orderId:"SF-1006",payment:"failed",verification:"approved",fulfillment:"awaiting_payment",docs:[]},
 paid:{title:"Paid and queued for fulfillment",reason:"Mock payment succeeded and fulfillment can begin.",state:"TX",orderId:"SF-1007",payment:"paid",verification:"approved",fulfillment:"processing",docs:[]}
};
