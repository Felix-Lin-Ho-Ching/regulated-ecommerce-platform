...
export const products = [
 ...
 { id:"p4", slug:"civicshield-safety-light", name:"CivicShield Safety Light", category:"visibility", price:39, status:"active", restricted:false, stock:92, sku:"CSL-400", description:"High-visibility light and whistle bundle for commuting and travel." }
] as const;
...
export const checkoutCases: Record<CheckoutOutcome, { title:string; reason:string; state:string; orderId:string; payment:string; verification:string; fulfillment:string; docs:string[] }> = {
 allowed:{title:"Allowed after automated checks",reason:"Destination and product category are covered by an allow rule.",state:"AZ",orderId:"CG-1001",payment:"not_collected",verification:"passed",fulfillment:"not_started",docs:[]},
 blocked:{title:"Blocked by destination rule",reason:"Mock rule blocks knuckle_stun_device shipments to NY. Payment is unavailable.",state:"NY",orderId:"CG-1002",payment:"not_collected",verification:"blocked",fulfillment:"blocked",docs:[]},
 pending_admin_review:{title:"Pending admin review",reason:"County-level rule needs manual review before payment.",state:"IL",orderId:"CG-1003",payment:"not_collected",verification:"manual_review",fulfillment:"on_hold",docs:[]},
 pending_document_upload:{title:"Document upload required",reason:"State rule requires government ID plus proof of residence before payment.",state:"CA",orderId:"CG-1004",payment:"not_collected",verification:"documents_required",fulfillment:"on_hold",docs:["Government ID","Proof of residence"]},
 ready_for_payment:{title:"Ready for hosted payment",reason:"Compliance and document review are approved. Continue to hosted payment.",state:"TX",orderId:"CG-1005",payment:"ready",verification:"approved",fulfillment:"awaiting_payment",docs:[]},
 payment_failed:{title:"Hosted payment failed",reason:"Processor returned a mock failure. Eligibility remains approved; retry payment.",state:"TX",orderId:"CG-1006",payment:"failed",verification:"approved",fulfillment:"awaiting_payment",docs:[]},
 paid:{title:"Paid and queued for fulfillment",reason:"Hosted payment succeeded and fulfillment can begin.",state:"TX",orderId:"CG-1007",payment:"paid",verification:"approved",fulfillment:"processing",docs:[]}
};
...
export const documentReviews = [{id:"DOC-91", order:"CG-1004", customer:"Sam Rivera", type:"Government ID", status:"pending", age:"2h"},{id:"DOC-92",order:"CG-1004",customer:"Sam Rivera",type:"Proof of residence",status:"pending",age:"2h"}];
export const launchGates = [{name:"Restricted product catalog",state:"ready",ownerOnly:false},{name:"Rule coverage for launch states",state:"blocked",ownerOnly:true},{name:"Hosted payment settings",state:"ready",ownerOnly:true},{name:"Legal policies published",state:"enabled",ownerOnly:false},{name:"Backup/export configured",state:"blocked",ownerOnly:true}];
