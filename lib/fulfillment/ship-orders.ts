...
export async function shipOrders(input: ShipOrdersInput): Promise<ShipOrdersResult> {
  ...
  if (result.shipped.length) {
    ...
    await Promise.all([
      ...orders.filter((o: any) => o.customerEmail).map((o: any) => logDebugEmail({ type: "CUSTOMER_SHIPMENT", to: o.customerEmail, subject: `Order ${o.orderNumber} shipped`, text: `Your order ${o.orderNumber} has been marked shipped. Carrier: ${tracking.carrier}. Tracking: ${tracking.trackingNumber}.`, orderId: o.id, metadata: { orderNumber: o.orderNumber, carrier: tracking.carrier, trackingNumber: tracking.trackingNumber } }).catch(() => undefined)),
      ...recipients.map((r: any) => logDebugEmail({ type: "INTERNAL_SHIPMENT", to: r.email, subject: `Shipment confirmed for ${result.shipped.length} order(s)`, text: `Shipment confirmed by ${input.actor.email} (${input.actor.role}). Carrier: ${tracking.carrier}. Tracking: ${tracking.trackingNumber}. Order IDs: ${result.shipped.join(", ")}.`, metadata: { orderIds: result.shipped, carrier: tracking.carrier, trackingNumber: tracking.trackingNumber, actingUserEmail: input.actor.email, actingRole: input.actor.role } }).catch(() => undefined)),
    ]);
  }
  return result;
}
