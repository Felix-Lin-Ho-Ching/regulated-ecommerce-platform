...
import type { PaymentOpaqueData, CardSummary } from "@/lib/payments/client/create-payment-opaque-data";
import type { PaymentAddress } from "@/lib/payments/gateways/authorize-net-types";

export function isApprovedPaymentMode() {
  return true;
}
...
export async function releaseOrderAfterPaymentApproval(orderId: string, actor?: { adminId?: string | null; email?: string | null; role?: string | null; demo?: boolean }) {
  const result = await (prisma as any).$transaction(async (tx: any) => {
    ...
    await tx.auditLog.create({ data: { actorAdminId: actor?.demo ? null : actor?.adminId ?? null, action: "PAYMENT", entityType: "Order", entityId: order.id, note: "Authorize.net-shaped payment approved.", metadata: { orderNumber: order.orderNumber, actingUserEmail: actor?.email, actingRole: actor?.role, paymentStatus: "APPROVED", fulfillmentStatus: "READY_TO_SHIP", provider: "AUTHORIZE_NET_MOCK" } } });
    return updated;
  });
...
export async function createOrderRequestFromCart(options: { opaqueData: PaymentOpaqueData; cardSummary: CardSummary; shippingAddress: PaymentAddress; billingAddress: PaymentAddress; customerEmail: string; dob?: { month: string; day: string; year: string } }): Promise<CustomerOrderSummary> {
  const [session, cart, shipping] = await Promise.all([
    getCustomerSession(),
    getCartSnapshot(),
    Promise.resolve({ name: `${options.shippingAddress.firstName} ${options.shippingAddress.lastName}`.trim(), line1: options.shippingAddress.line1, line2: options.shippingAddress.line2, city: options.shippingAddress.city, state: options.shippingAddress.state, postalCode: options.shippingAddress.postalCode, phone: options.shippingAddress.phone }),
  ]);
  ...
  if (isDatabaseConfigured) {
    ...
    const paymentResult = await (prisma as any).$transaction(async (tx: any) => processOrderPayment(tx, { id: readyOrder.id, orderNumber: readyOrder.orderNumber, totalCents: readyOrder.totalCents }, { opaqueData: options.opaqueData, billingAddress: options.billingAddress, shippingAddress: options.shippingAddress, customerEmail: options.customerEmail }));

    await prisma.auditLog.createMany({ data: [
      { action: "CREATE", entityType: "Order", entityId: order.id, note: "Checkout order received.", metadata: { status: "ORDER_REQUEST_SUBMITTED" } },
      { action: "UPDATE", entityType: "Order", entityId: order.id, note: "Auto eligibility checks passed.", metadata: { status: "AUTO_ELIGIBLE" } },
      { action: "PAYMENT", entityType: "Order", entityId: order.id, note: "Authorize.net-shaped payment gateway processed checkout payment.", metadata: { status: readyOrder.status, paymentStatus: paymentResult.paymentAttempt.status, paymentCollected: paymentResult.paymentAttempt.status === "APPROVED", fulfillmentReleased: false, card: options.cardSummary } },
    ] });

    if (paymentResult.paymentAttempt.status === "APPROVED") {
    ...
    return {
      ...
      status: readyOrder.status === "PAID" ? "Paid" : readyOrder.status === "PAYMENT_FAILED" ? "Payment failed" : "Payment failed",
      total: readyOrder.totalCents / 100,
      payment: readyOrder.status === "PAID" ? "Payment collected" : customerPaymentStatusLabel(paymentResult.paymentAttempt.status),
